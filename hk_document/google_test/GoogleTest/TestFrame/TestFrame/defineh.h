...
void Fail(const char* FuncName,const char* Param, int line)
{
	printf("line: %d,  Function %s  return failed,   param:%s \n", line, FuncName, Param);
	suc=false;
}
...
#define FAIL_L(funcName,Param) Fail(funcName,Param, __LINE__)
...
#define ASSERT_VAL(exp, funcName, Param)		\
		if(false==(exp)){				\
			FAIL_L(funcName,Param);				\
		}
