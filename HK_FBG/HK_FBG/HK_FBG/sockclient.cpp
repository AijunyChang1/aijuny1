...
#else
	m_destinationSockAddr.sin_family = AF_INET;
	m_destinationSockAddr.sin_addr.S_un.S_addr = htonl(INADDR_ANY);
	m_destinationSockAddr.sin_port = htons(m_remote_port);
	bind(m_sockettcp,(SOCKADDR*)&m_destinationSockAddr,sizeof(SOCKADDR));
	listen(m_sockettcp,10);
...
int SockClient::ConnectTo()
{
	...
	*/
	DWORD targetThreadID2;
	HANDLE HACCThread;                          		
    HACCThread=CreateThread(NULL,0,AcceptThread,this,0,&targetThreadID2);
...
#ifdef FBG
DWORD WINAPI SockClient::AcceptThread(LPVOID Param)
{
	...
	while(1)
	{
	    ...
	    SOCKET sockConn = accept(P->m_sockettcp,(SOCKADDR*)&P->acceptSockAddr, &len);
		P->m_accetpsocket = sockConn;
		sprintf(w_log, "Welcome %s to here!", inet_ntoa(P->acceptSockAddr.sin_addr));
		write_log(w_log);
		DWORD targetThreadID;
	    HANDLE HCliThread;                          		
        HCliThread=CreateThread(NULL,0,P->CliListenThread,P,0,&targetThreadID);
	    CloseHandle(HCliThread);
...
DWORD WINAPI SockClient::CliListenThread(LPVOID Param)
{
	...
	while(1)
	{
		...
		int l_iLen=0;
#ifndef FBG
		l_iLen=recv(P->m_sockettcp,(char*)MessageBuf,BUF_SIZE-1,0);
#else
		l_iLen=recv(P->m_accetpsocket,(char*)MessageBuf,BUF_SIZE-1,0);
		

#endif
		if ((l_iLen<0)||(l_iLen==0))
		{     
#ifdef FBG
...
# endif
		...
		memcpy(tmp_ptr, MessageBuf, l_iLen);
		iLen = l_iLen + P->m_remain_len;
		P->m_remain_len = 0;
		...
		int data_n;
		Sensor_Node* data_list = NULL;
        do
		{
		    has_error = parse_fbg_msg(&msg_ptr, iLen, &dev_id[0], &rec_time[0], data_n, &data_list); 
			if(has_error==1)
			{
		    ...
		    if (!has_error)
		    {
			   	sprintf(w_log, "Parsed a message, dev_id: %s,rec_time: %s, data_num: %d", dev_id, rec_time,  data_n);
				write_log(w_log);
				Sensor_Node * temp_node;
				while (data_list)
				{
					FBGQueueElem* new_elem = new FBGQueueElem;
					if (new_elem!=NULL)
					{
						new_elem->dev_id = dev_id;
						new_elem->sensor_id=data_list->sensor_id;
						new_elem->rec_time=rec_time;
						new_elem->value = data_list->data;
						new_elem->next = NULL;
...
DWORD WINAPI SockClient::HandleQueue(LPVOID Param)
{
	...
	while (1)
	{
        ...
        if(this_ptr!=NULL)
		{
	                         ...
	                         memset(&tmp[0], 0, sizeof(tmp));
				             strftime(tmp, sizeof(tmp), "%Y-%m-%d %X",localtime(&t));
						     sql= "insert into hk_fbg_real_data_info(dev_id,create_time, sor_code,data) values (";
							 sql = sql + "'"+this_ptr->dev_id+"',";
							 sql = sql + "'"+this_ptr->rec_time+"',";
							 sql = sql +"'"+ this_ptr->sensor_id+"',";
							 sprintf(itocode,"%f",this_ptr->value);
							 sql = sql +"'"+ itocode+"')";
